...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        ############################################################################
        H1,cache['H1']          = affine_forward(X , self.params['W1'], self.params['b1']) 
        scores,cache['scores']    = affine_forward(H1 , self.params['W2'], self.params['b2'])
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        sm_loss ,cache['dout']   = softmax_loss(scores, y)
        reg_loss = self.reg * 0.5 *(np.sum(self.params['W1']*self.params['W1']) +   np.sum(self.params['W2']*self.params['W2']) )
        loss     = sm_loss  + reg_loss 
        cache['dH1'], grads['W2'], grads['b2']= affine_backward(cache['dout'],cache['scores'])
        _ , grads['W1'], grads['b1']= affine_backward(cache['dH1'],cache['H1'])
...
##        self.params = {}
##        
##        self.layer_dims.extend(hidden_dims)
##        self.layer_dims.extend([num_classes])
##        
##        ############################################################################
...
##            i = l + 1
##            self.params['W' + str(i)] = np.random.normal(scale = weight_scale,  size =(input_dim,hidden_dims[l]))
##            self.params['b' + str(i)] = np.zeros(hidden_dims[l])
##            self.params['gamma' + str(i)] = np.array([1])
##            self.params['beta'  + str(i)] = np.array([0])
##        #output layers params
##        self.params['W' + str(l+2)] = np.random.normal(scale = weight_scale,  size =(input_dim,hidden_dims[l]))
##        ############################################################################
##        #                             END OF YOUR CODE                             #
##        ############################################################################
##
...
##            j = i + 1
##            layer_out , cache['layer' + str(j) + 'affine']= affine_forward(layer_out, self.params['W' + str(j)], self.params['b' + str(j)])
##            if self.normalization is not None :
##                if self.normalization=='batchnorm':
##                    layer_out , cache['layer' + str(j) + 'nrom']= batchnorm_forward(layer_out, self.params['gamma' + str(j)], self.params['beta' + str(j)], self.bn_params[i])
##            layer_out , cache['layer' + str(j) + 'relu']= relu_forward(layer_out)
##            if self.use_dropout:
##                layer_out , cache['layer' + str(j) + 'dropout']= dropout_forward(layer_out, self.dropout_params[i])
##        scores = affine_forward(layer_out)
##        ############################################################################
##        #                             END OF YOUR CODE                             #
##        ############################################################################
##
...
##        # of 0.5 to simplify the expression for the gradient.                      #
##        ############################################################################
##        sm_loss , dout = softmax(scores,y)
##        reg_loss  = 0
##        for key , value in self.params.iteritems():
##            if key[0] == 'W':
##                reg_loss += self.reg * 0.5 * np.sum(value*value)
##        
##        loss    = sm_loss + reg_loss
##
##        upstream_grad = dout
##        ## compute grads 
##        for i in range(self.num_layers - 1).reverse():
##            j = i + 1
##            if self.use_dropout:
##                upstream_grad = dropout_backward(upstream_grad, cache['layer' + str(j) + 'dropout'])
##            upstream_grad = relu_forward(upstream_grad, cache['layer' + str(j) + 'relu'])
##        ############################################################################
##        #                             END OF YOUR CODE                             #
##        ############################################################################
##
